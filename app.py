...
window = tk.Tk()
window.title('Bingo')
positionRight = int(window.winfo_screenwidth()/2 - 650/2)
window.geometry(f"650x650+{positionRight}+10")
window.config(bg='#d9d9d9')

for i, item in enumerate(['B','I','N','G','O']):
    label = tk.Label(window, text=item, font=('Consolas', 20), fg='#ff0000', bg='#d9d9d9')
    label.grid(row=0, column=i+1, sticky='nesw', pady=3)
...
for i in range(1, 76):
    button = tk.Button(window, text=i, font=('Consolas', 17), pady=30, padx=50, bg='#d9d9d9')
...
label = tk.Label(window, text='Pressione F5 para iniciar um novo jogo', font=('Consolas', 13), fg='#0000ff')
